...
class Solution {
    ...
    fun addBinary(a: String, b: String): String {
        val (large, small) = if (a.length > b.length) a to b else b to a
        val reversedRes = StringBuilder()
        var carry = 0

        for (i in large.indices) {
            val bit1 = large.let { it[it.lastIndex - i].digitToInt() }
            val bit2 = small.let { it.getOrNull(it.lastIndex - i)?.digitToInt() } ?: 0
