...
class Solution {
    ...
    fun minimumDeleteSum(s1: String, s2: String): Int {
        ...
        // Base cases that all characters of s1[:lengthPrefix1] are deleted since s2[:0] is empty
        for (lengthPrefix1 in 1..s1.length) {
            dp[lengthPrefix1][0] = s1[lengthPrefix1 - 1].ascii() + dp[lengthPrefix1 - 1][0]
        }
        // Base cases that all characters of s2[:lengthPrefix2] are deleted since s1[:0] is empty
