...
class Solution {
    ...
    fun minimumDifference(nums: IntArray): Long {
        ...
        // nums[0..<n+i].
        val nSmallestSumL = LongArray(n + 1)
        val nSmallestL = PriorityQueue<Int>(reverseOrder())

        for (j in 0..<n) {
            nSmallestL.add(nums[j])
            nSmallestSumL[0] += nums[j]
        }
        for (i in 1..n) {
            nSmallestL.add(nums[n + i - 1])
            nSmallestSumL[i] = nSmallestSumL[i - 1] + nums[n + i - 1] - nSmallestL.poll()
        }

        // nLargestSumR[i]:= the sum of the n largest elements from
        // nums[n+i..<3n].
        var nLargestSumR = 0L // Initialized with i = n
